...
function isMissingRouteColumnError(error) {
  ...
  return message.includes("route_history") || message.includes("route_stops");
}

function withoutRouteColumns(row) {
  const compatibleRow = { ...row };
  delete compatibleRow.route_stops;
  delete compatibleRow.route_history;
...
export async function saveRegistration(registrationData) {
  ...
  if (isSupabaseConfigured && supabase) {
    ...
    let result = await supabase.from("registrations").upsert(row).select().single();
    if (result.error && isMissingRouteColumnError(result.error)) {
      result = await supabase.from("registrations").upsert(withoutRouteColumns(row)).select().single();
    }
  ...
  if (isSupabaseConfigured && supabase) {
    ...
    if (result.error && isMissingRouteColumnError(result.error)) {
      result = await supabase
        .from("registrations")
        .update(withoutRouteColumns(rowUpdates))
        .eq("id", cleanId)
        .select()
        .maybeSingle();
    }
