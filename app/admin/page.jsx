...
export default function AdminDashboardPage() {
  ...
  const formatRoute = (r) => {
    if (!r) return "-";
    if (Array.isArray(r.routeStops) && r.routeStops.length > 0) {
      return r.routeStops.join(" → ");
    }
    return `${r.travelFrom || "-"} → ${r.travelTo || "-"}`;
  };
              ...
              {[
                {
                  ...
                  fields: [
                    ...
                    { label: "Route", value: formatRoute(selectedRegistration) },
              ...
              >
                ...
                {qrVerifyResult.registration && (
                  <div style={{ paddingTop: "10px", borderTop: "1px solid #bae6fd", display: "flex", flexDirection: "column", gap: "4px", fontFamily: "monospace", fontSize: "11px", color: "#0f172a" }}>
                    ...
                    <p style={{ margin: 0 }}><strong style={{ color: "#0284c7" }}>Route:</strong> {formatRoute(qrVerifyResult.registration)}</p>
                  </div>
                )}
              </div>
            )}
